...
exports.clearHistory = exports.parseTransactionsFromText = exports.analyzeScreenshot = exports.askAI = void 0;
...
const gigaAxios = axios_1.default.create({
...
const getToken = async () => {
    ...
    if (!auth)
        throw new Error('GIGACHAT_AUTH не задан в переменных окружения');
    const res = await gigaAxios.post(GIGACHAT_TOKEN_URL, 'scope=GIGACHAT_API_PERS', {
        headers: {
    ...
    _tokenExpiry = res.data.expires_at * 1000;
    return _token;
};
// ─── System prompt ────────────────────────────────────────────────────────────
...
// ─── Fuzzy match helper ───────────────────────────────────────────────────────
const fuzzyMatchItems = (items, word) => {
    const w = word.toLowerCase();
    return (items.find(i => i.name.toLowerCase() === w) ||
    ...
    try {
        if (name === 'create_transaction') {
            const [categories, accounts] = await Promise.all([
                prisma_1.prisma.category.findMany({ where: { isArchived: false } }),
                prisma_1.prisma.account.findMany({ where: { isArchived: false }, orderBy: { createdAt: 'asc' } }),
            ]);
            ...
            const account = args.accountName
                ...
                : accounts[0];
            if (!account)
                return 'Ошибка: нет доступных счетов';
            ...
            return `Записано: ${sign}${tx.amount} ₽ · ${tx.category?.name || 'без категории'} · ${tx.account.name} · ${(0, date_fns_1.format)(tx.date, 'dd.MM.yyyy')}`;
        }
        if (name === 'create_budget') {
            ...
            await prisma_1.prisma.budget.upsert({
        ...
        if (name === 'plan_budgets_next_month') {
            ...
            const prevMonthDate = (0, date_fns_1.subMonths)(now, 1);
            const prevStart = (0, date_fns_1.startOfMonth)(prevMonthDate);
            const prevEnd = (0, date_fns_1.endOfMonth)(prevMonthDate);
            ...
            txs.forEach(tx => {
                if (tx.category) {
                    if (!catSpend[tx.category.id])
                        catSpend[tx.category.id] = { id: tx.category.id, name: tx.category.name, total: 0 };
                    catSpend[tx.category.id].total += tx.amount;
                }
            });
            const FIXED_CATEGORIES = ['квартира', 'кредит', 'кредиты', 'аренда'];
            const isFixed = (name) => FIXED_CATEGORIES.some(f => name.toLowerCase().includes(f));
            ...
            const prevLabel = (0, date_fns_1.format)(prevMonthDate, 'MMMM yyyy');
            if (results.length === 0)
                return `Нет трат в ${prevLabel} для планирования бюджетов.`;
        ...
        if (name === 'plan_budget_from_total') {
            ...
            txs.forEach(tx => {
                if (tx.category) {
                    if (!catTotals[tx.category.id])
                        catTotals[tx.category.id] = { id: tx.category.id, name: tx.category.name, total: 0 };
            ...
            if (entries.length === 0) {
                const threeMonthsAgo = (0, date_fns_1.startOfMonth)((0, date_fns_1.subMonths)(now, 3));
                const fallbackTxs = await prisma_1.prisma.transaction.findMany({
                ...
                fallbackTxs.forEach(tx => {
                    if (tx.category) {
                        if (!fallbackTotals[tx.category.id])
                            fallbackTotals[tx.category.id] = { id: tx.category.id, name: tx.category.name, total: 0 };
                        fallbackTotals[tx.category.id].total += tx.amount;
                    }
                });
                entries = Object.values(fallbackTotals).filter(c => c.total > 0);
                if (entries.length === 0)
                    return 'Нет данных о тратах для распределения бюджета.';
            }
            const FIXED_CATEGORIES = ['квартира', 'кредит', 'кредиты', 'аренда'];
            const isFixed = (name) => FIXED_CATEGORIES.some(f => name.toLowerCase().includes(f));
        ...
        if (name === 'get_transactions_by_category') {
            ...
            const periodStart = (0, date_fns_1.startOfMonth)((0, date_fns_1.subMonths)(now, months - 1));
            const periodEnd = (0, date_fns_1.endOfDay)(now);
            const txs = await prisma_1.prisma.transaction.findMany({
            ...
            if (txs.length === 0)
                return `Нет транзакций по категории "${category.name}" за указанный период.`;
            const total = txs.reduce((s, t) => s + t.amount, 0);
            const lines = txs.map(t => {
                const sign = t.type === 'income' ? '+' : '-';
                const comment = t.comment ? ` — ${t.comment}` : '';
                return `  ${(0, date_fns_1.format)(t.date, 'dd.MM')} ${sign}${t.amount} ₽${comment}`;
            });
            return `Транзакции по категории "${category.name}" (${(0, date_fns_1.format)(periodStart, 'dd.MM')}–${(0, date_fns_1.format)(now, 'dd.MM.yyyy')}):\n${lines.join('\n')}\n\nИтого: ${total.toFixed(0)} ₽, ${txs.length} операций`;
        }
        if (name === 'delete_last_transaction') {
            const last = await prisma_1.prisma.transaction.findFirst({ orderBy: { date: 'desc' } });
            if (!last)
                return 'Нет транзакций для удаления.';
            await prisma_1.prisma.transaction.delete({ where: { id: last.id } });
            const sign = last.type === 'income' ? '+' : '-';
            return `Удалена последняя транзакция: ${sign}${last.amount} ₽ от ${(0, date_fns_1.format)(last.date, 'dd.MM.yyyy')}`;
        }
...
const getFinancialContext = async () => {
    ...
    const prev1Date = (0, date_fns_1.subMonths)(now, 1);
    const prev1Start = (0, date_fns_1.startOfMonth)(prev1Date);
    const prev1End = (0, date_fns_1.endOfMonth)(prev1Date);
    // Accounts + balances
    ...
    const computeBalance = (acc) => {
        ...
        for (const tx of acc.transactions) {
            if (tx.type === 'income')
                ...
                bal -= tx.amount;
            else if (tx.type === 'transfer' && tx.accountId === acc.id)
                bal -= tx.amount;
        }
        for (const tx of acc.transfersTo)
            bal += tx.amount;
        return bal;
    };
    const accountLines = accounts.map(a => `  ${a.name} (${a.type}): ${computeBalance(a).toFixed(2)} ₽`).join('\n');
    ...
    const [todayTx, yesterdayTx] = await Promise.all([
        prisma_1.prisma.transaction.findMany({ where: { date: { gte: todayStart, lte: todayEnd } }, include: { category: { select: { name: true } } } }),
        prisma_1.prisma.transaction.findMany({ where: { date: { gte: yesterdayStart, lte: yesterdayEnd } }, include: { category: { select: { name: true } } } }),
    ]);
    const today = summarizeDay(todayTx);
    const yesterday = summarizeDay(yesterdayTx);
    // Last 7 days
    const week7Start = (0, date_fns_1.subDays)(now, 6);
    const weekTx = await prisma_1.prisma.transaction.findMany({
    ...
    const weekCatLines = Object.entries(weekCatSpend)
        ...
        .join('\n');
    // Current month
    const currentTx = await prisma_1.prisma.transaction.findMany({
    ...
    const prevTx = await prisma_1.prisma.transaction.findMany({
    ...
    // Goals
    const goals = await prisma_1.prisma.goal.findMany({ where: { isCompleted: false } });
    const goalLines = goals.map(g => {
        const pct = g.targetAmount > 0 ? Math.round((g.currentAmount / g.targetAmount) * 100) : 0;
        return `  ${g.name}: ${g.currentAmount.toFixed(0)} ₽ из ${g.targetAmount.toFixed(0)} ₽ (${pct}%)`;
    }).join('\n');
    return `
ТЕКУЩИЕ ФИНАНСОВЫЕ ДАННЫЕ (на ${(0, date_fns_1.format)(now, 'dd.MM.yyyy')}):

Счета:
${accountLines || '  —'}
Общий баланс: ${totalBalance.toFixed(2)} ₽

Сегодня (${(0, date_fns_1.format)(now, 'dd.MM')}):
  Доходы: ${today.incTotal.toFixed(2)} ₽
  Расходы: ${today.total.toFixed(2)} ₽
${today.catStr || '  —'}

Вчера (${(0, date_fns_1.format)((0, date_fns_1.subDays)(now, 1), 'dd.MM')}):
  Доходы: ${yesterday.incTotal.toFixed(2)} ₽
  Расходы: ${yesterday.total.toFixed(2)} ₽
${yesterday.catStr || '  —'}

Последние 7 дней (${(0, date_fns_1.format)(week7Start, 'dd.MM')}–${(0, date_fns_1.format)(now, 'dd.MM')}):
  Доходы: ${weekIncome.toFixed(2)} ₽
  Расходы: ${weekExpense.toFixed(2)} ₽
Расходы по категориям за 7 дней:
${weekCatLines || '  —'}

Текущий месяц (${(0, date_fns_1.format)(monthStart, 'MMMM yyyy')}):
...
Прошлый месяц (${(0, date_fns_1.format)(prev1Date, 'MMMM yyyy')}) — основа для планирования бюджетов:
...
// ─── Main AI function ─────────────────────────────────────────────────────────
const askAI = async (chatId, userMessage) => {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const res = await gigaAxios.post(`${GIGACHAT_API}/chat/completions`, {
        ...
        if (isFunctionCall && msg.function_call) {
            ...
            const fnArgs = typeof rawArgs === 'string' ? JSON.parse(rawArgs) : rawArgs;
    ...
    if (!finalReply)
        finalReply = 'Слишком много шагов, попробуй переформулировать запрос.';
...
const TEXT_PARSE_PROMPT = (text) => `Вот текст, распознанный со скриншота банковского приложения:
...
const analyzeScreenshot = async (base64Image) => {
    const token = await getToken();
    const messages = [
...
exports.parseTransactionsFromText = parseTransactionsFromText;
